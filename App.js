...
const URL = "https://detect.roboflow.com/road-sign-detection-gmkcf/3"; // copy and paste your Theos deployment URL here
const FALLBACK_URL = "";
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 0 || error.response.status === 413)
        throw new Error("image too large, please select an image smaller than 25MB.");
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.controls}>
        ...
        >
          {image ? (
            <Button title="Retake" onPress={retake} icon="ios-reload" />
          ) : (
            <Button
              title=""
              icon="camera-reverse"
          ...
          <Button disabled={detected} onPress={takePicture} icon="ios-camera" />
        </View>
      </View>
    </View>
  );
